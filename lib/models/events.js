...
Events.handleWaitForDockRemoved = function (data, cb) {
  log.info({ data: data }, 'Events.handleWaitForDockRemoved');
  Consul.waitForDockRemoved(data.dockerUrl, function (err) {
    if (err) { return cb(err); }

    log.trace({ data: data }, 'handleWaitForDockRemoved publishing dock-removed');
