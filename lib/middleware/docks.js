...
function update (req, res, next) {
  log.debug({ query: req.query, body: req.body }, 'Updating docks');
  ...
  if (!host) {
    return next(errorCat.create(400, 'host param is required'));
  }
  if (!isHostUrlValid(host)) {
    return next(errorCat.create(
      400, 'host must be in form http://1.1.1.1:4242', host
    ));
  }

  if (!key) {
    return next(errorCat.create(400, 'key param is required'));
  }
  if (!isKeyValid(key)) {
    return next(errorCat.create(400, 'key param is invalid'));
  }

  if (!value) {
    return next(errorCat.create(400, 'value param is required'));
  }
