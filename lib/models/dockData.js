 ...
 *    tags             comma-separated org id and type of dock (run, build)
 *
...
/**
 * Find dock data using `host`
 * @param {String} host - find dock data using host
 * @param {function} cb callback to execute once the dock data was fetched
 */
function getDockByHost (host, cb) {
  log.debug({ host: host }, 'getDockByHost')
  redis.client.hgetall(augmentHost(host), cb)
}
...
function getAllDocks (cb) {
  ...
  redis.client.keys(host, function (err, keys) {
  ...
  getAllDocks(function (err, docks) {
    ...
    if (typeof tags === 'string') {
      ...
      docks = docks.filter(function(dock) {
        ...
        // ensure every tag requested is in the dock's tag array
        return tagArray.every(function(currentTag) {
          return ~dockTagArray.indexOf(currentTag);
        });
      });
    } else {
      log.debug({
...
function incKey (host, type, amount, cb) {
  ...
  redis.client.exists(host, function(err, data) {
    if (err) { return cb(err); }
    if (data === '0') { return cb(errorCat.create(404, 'key does not exist')); }
    redis.client.hincrby(host, key, amount, cb);
  });
}
...
function setKey (host, field, value, cb) {
  ...
  host = augmentHost(host);
  redis.client.hset(host, field, value, cb);
}
