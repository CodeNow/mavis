...
module.exports.worker = function (data, done) {
  ...
  workerDomain.on('error', function (err) {
    log.fatal({
      ...
      data: data,
      err: err
    }, 'on-dock-unhealthy domain error');
    error.log('domain uncaught exception', {
      data: data,
      err: err
    });
...
OnDockUnhealthyWorker.prototype.handle = function (data, cb) {
  ...
  dockData.deleteHost(data.host, function (err) {
    if (err) {
      log.error(logData, 'deleteHost failed');
      error.log('deleteHost failed', data);
      return cb();
    }
    rabbitMQ.hermesClient.publish('on-dock-removed', data);
    rabbitMQ.hermesClient.publish('cluster-instance-provision', data);
    cb();
  });
};
