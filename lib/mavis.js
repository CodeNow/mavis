...
function obtainOptimalHost (hint, cb) {
  ...
  if (!hint.type) {
    return cb(errorCat.create(400, 'No hint type specified'));
  }

  if (isValidHintType(hint.type)) {
    return cb(errorCat.create(400, 'Cannot handle type: ' + hint.type));
  }
  ...
  if (isTypeRandom && hasPreviousDock) {
    return cb(errorCat.create(400, 'Cannot handle type ' + hint.type + ' with prevDock'));
  }

  log.trace('Fetching all hosts.');
  dockData.getValidDocks(hint.tags, function (err, currentStates) {
    if (err) { return cb(err); }
    if (currentStates.length <= 0) {
      return cb(errorCat.create(503, 'no docks avalible', { hint: hint }));
    }

    if (typeof hint.prevDock === 'string') {
      var foundDock = false;
      currentStates.forEach(function (dock) {
        if (dock.host === hint.prevDock) {
          foundDock = true;
        }
      });
      if (foundDock) {
        log.trace('Hint provided a previous dock and dock still exists');
        return dockData.incKey(hint.prevDock, hint.type, 1, function (err) {
          cb(err, hint.prevDock);
        });
      }
      log.trace('Hint provided a previous dock, but the dock no longer exists.');
    }

    log.trace('Finding optimal host');
    processData(hint, currentStates, function (err, host) {
      if (err) { return cb(err); }
      if (isTypeRandom) { return cb(null, host); }
      monitor.event({
        title: 'mavis.host.selected',
        text: hint.type,
        hostname: host,
        tags: (hint.tags) ? hint.tags.split(',') : null
      });
