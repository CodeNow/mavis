...
Events.handleUnhealthy = function (data, cb) {
  ...
  if (!Events._hasValidHost(data)) {
    var noHostError = new TaskFatalError(
      'on-dock-unhealthy',
      'Failed validation: no host',
      { data: data }
    );
    return cb(noHostError);
  }
  dockData.getDockByHost(data.host, function (err, dock) {
    if (err || !dock || !dock.tags) {
      var noDockError = new TaskFatalError(
        'on-dock-unhealthy',
        'Dock is not in mavis anymore',
        { data: data, err: err }
      );
      return cb(noDockError);
    }
    var dockTagArray = dock.tags.split(',') || [];
    var githubId = dockTagArray[0]
    if (!githubId) {
      var noGithubIdError = new TaskFatalError(
        'on-dock-unhealthy',
        'No github id was found',
        { data: data }
      );
      return cb(noGithubIdError)
    }
    dockData.deleteHost(data.host, function (err) {
      if (err) {
        var taskErr = new TaskError('on-dock-unhealthy', 'Failed to delete host', err);
        return cb(taskErr)
      }
 ...
 *                    data.githubId url to check for format: 2335750
